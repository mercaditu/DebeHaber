      ...
      $details = collect();
       ...
       foreach ($raw as $result)
       {
         ...
         if ($result->DocumentType == '1') {
					 ...
					 $detail['tipo'] = array_search($result->DocumentType, static::ARANDUKA_MAP, true);
					 $detail['relacionadoTipoIdentificacion'] = 'RUC';
					 $detail['fecha'] = date_format($date, 'd-m-Y');
					 $detail['id'] = $result->ID;
					 $detail['ruc'] = $taxPayer->taxid;
					 $detail['egresoMontoTotal'] = $result->Value;
					 $detail['relacionadoNombres'] = $result->Partner;
					 $detail['relacionadoNumeroIdentificacion'] = $result->PartnerTaxID;
					 $detail['timbradoCondicion'] = $result->PaymentCondition != "0" ? 'credit' : 'contado';
					 $detail['timbradoDocumento'] = $result->Number;
					 $detail['timbradoNumero'] = $result->Code;
					 $detail['tipoEgreso'] = 'gasto';
					 $detail['tipoEgresoTexto'] = 'Gasto';
					 $detail['tipoTexto'] = 'Factura';
					 $detail['subtipoEgreso'] = $result->ChartCode;
					 $detail['subtipoEgresoTexto'] = $result->ChartName ?? 'Gastos personales y de familiares a cargo realizados en el país';
         } else {
					 $detail = collect();
					 $detail['periodo'] = date_format($date, 'Y');
 					 $detail['tipo'] = array_search($result->DocumentType, static::ARANDUKA_MAP, true);
					 ...
					 $detail['timbradoDocumento'] = $result->Number;
 					 ...
 					 $detail['tipoEgresoTexto'] = 'Gasto';
 					 $detail['tipoTexto'] = 'Factura';
 					 $detail['subtipoEgreso'] = 'GPERS';
 					 $detail['subtipoEgresoTexto'] = '';
					 $detail['numeroDocumento'] = $result->Number;
         }
         $details->add($detail);
