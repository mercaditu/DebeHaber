        ...
        <tbody>
            @foreach ($data->groupBy('salesID') as $group)
                <tr @if ($group->first()->status == 3) class="danger" @endif>
                    ...
                    <td class="important">
                        <a href="/{{ request()->route('taxPayer')->id }}/{{ request()->route('cycle')->id}}/commercial/sales/{{ $group->first()->salesID }}" target="_blank">
                            {{ $group->first()->number }}
                        </a>
                    </td>
            ...
            <tr class="group">
                ...
                <td class="number">
                    <b>{{ number_format($data->where('status', '!=', 3)->where('coefficient', '=', 0.1)->sum('vatValue'), 0, ',', '.') }}</b>
                </td>
                <td class="number">
                ...
                <td class="number">
                    <b>{{ number_format($data->where('status', '!=', 3)->where('coefficient', '=', 0.05)->sum('vatValue'), 0, ',', '.') }}</b>
                </td>
                <td class="number">
                ...
                <td class="number">
                    <b>{{ number_format($data->where('status', '!=', 3)->where('coefficient', '=', 0.00)->sum('vatValue'), 0, ',', '.') }}</b>
                </td>
                <td class="number">
                    <b>{{ number_format($data->where('status', '!=', 3)->sum('localCurrencyValue'), 0, ',', '.') }}</b>
                </td>
                <td></td>
                <td></td>
            </tr>
