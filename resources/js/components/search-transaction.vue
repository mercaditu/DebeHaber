<template>
    <div>
        <b-input-group>
                <b-input type="text" v-model="transactionNumber" placeholder="Search for Transaction Number" @keyup="searchTransactionNumber()"/>
               <b-input type="text" v-model="searchvalue" readonly="true"/>
        </b-input-group>
       
        <b-list-group>
            <b-list-group-item  v-for="transaction in transactions" @click="select(transaction)" :key="transaction.id">
                <b>{{ transaction.number }}</b> | {{ transaction.total }}
            </b-list-group-item>
        </b-list-group>
    </div>
</template>
    ...
    methods: {
        ...
        searchTransactionNumber()
        {
            ...
            if (app.searchnumber.length < 3) {
                ...
                crud.methods
                    .onRead(app.baseUrl + "/search/Purchasetransactions/" + app.searchnumber)
