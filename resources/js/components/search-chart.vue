<template>
    <div>
        <b-input-group>
                <b-input type="text" v-model="chartCode" placeholder="Search for Chart Code" @keyup="searchChartcode()"/>
                <b-input type="text" v-model="chartName" placeholder="Search for Chart Name" @keyup="searchChartname()"/>
        </b-input-group>
        <b-list-group>
            <b-list-group-item  v-for="chart in charts" @click="select(chart)" :key="chart.id">
                <b>{{ chart.name }}</b> | {{ chart.code }}
            </b-list-group-item>
        </b-list-group>
    </div>
</template>
    ...
    methods: {
        ...
        searchChartname()
        {
            ...
            var app = this;
               ...
               console.log(app.baseUrl + "/search/chartName/" + app.searchname);
                crud.methods
