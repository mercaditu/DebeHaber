<template>
  <div>
    <b-card no-body>
      <b-tabs pills card vertical>
        ...
        <b-tab title="File Import">
          <b-card class="app">
            <h3>
              Example - Import file with required login, firstname, lastname and
              optional values
            </h3>
            <br />
            <xls-csv-parser
        ...
        <b-tab title="Online Service Import" active>
          <b-row>
            ...
            <b-col>
              <b-button v-b-modal.integration-form class="float-right"
                >Create Integration</b-button
              >
            </b-col>
          </b-row>
          <b-table
            :items="data.integrationservice"
            :fields="data.integrationfields"
            striped
          >
            <template v-slot:cell(actions)="data">
              <b-button @click="get_data(data.item)">Fetch Data</b-button>
              <b-button @click="delete_configuration(data.item)"
                >Delete</b-button
              >
            </template>
          </b-table>
          ...
          <b-modal
            id="integration-form"
            title="Integration Service Form"
            size="lg"
          >
            <b-row>
              <b-col>
    ...
    <b-card v-if="data.data.length > 0">
      ...
      <div v-for="error in data.error" v-bind:key="error.index">
        <p class="danger">
          {{ error }}
        </p>
      </div>
        ...
        .then(function(response) {
          if (response.status === 200) {
            ...
            if (response.data != null) {
              response.data.sales.forEach(element => {
                app.data.data.push(element);
              });
              response.data.error.forEach(element => {
                app.data.error.push(element);
              });
